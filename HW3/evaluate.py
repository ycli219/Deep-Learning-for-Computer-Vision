...
class CLIPScore:
    ...
    def __call__(self, predictions, images_root):
        ...
        for img_name, pred_caption in predictions.items():
            ...
            if self.getCLIPScore(image, pred_caption[:]) > max_score:
                max_score = self.getCLIPScore(image, pred_caption[:])
                max_name = img_name
            if self.getCLIPScore(image, pred_caption[:]) < min_score:
                min_score = self.getCLIPScore(image, pred_caption[:])
                min_name = img_name
        print(max_score)
        print(max_name)
        print(min_score)
